...
tests_to_run_file=tests_to_run.$$
find . -mindepth 1 -maxdepth 1 -type d | sed -E -e 's/^\.\///' > "$tests_to_run_file"
declare -a tests_to_run
readarray -t tests_to_run < "$tests_to_run_file"
rm "$tests_to_run_file"
for d in "${tests_to_run[@]}"; do
  if [ -f "$MYDIR/$d/test.sh" ]; then
